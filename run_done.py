...
def parse_args():
    ...
    parser.add_argument('--config', nargs='?',
                        default='config_done', help='Input config path')
...
def get_total_loss(model, sess, x_train, y_train, indexes, o1, o2, o3, batch_size):
    batcher = batch_iter(x_train, y_train, indexes, o1, o2,
                         o3, batch_size, num_epochs=1, shuffle=False)
    L1, L2, L3 = [], [], []
    L4, L5 = [], []
...
def trainer(sess, model, x_train, y_train, num_epochs):
    with sess.as_default():
        ...
        batcher = batch_iter(x_train, y_train, indexes, o1,
                             o2, o3, batch_size, num_epochs)
        ...
        for feed_dict, order, epoch_end in batcher:
            ...
            model.train_step(sess, feed_dict, start_align=(
                epoch > config['pretrain_threshold']), print_this=False)
            if epoch_end:
                if epoch > config['pretrain_threshold']:
                    l1, l2, l3 = get_total_loss(
                        model, sess, x_train, y_train, indexes, o1, o2, o3, batch_size)
        ...
        np.savetxt(os.path.join(
            'emb', config['experiment_name'] + '.emb'), final)
...
print('Data load complete. Structure input size',
      Adj.shape, 'Content input size', Con.shape)
