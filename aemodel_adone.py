...
class AutoEncoder(object):
    ...
    def _add_placeholders(self):
        self.input_x = tf.placeholder(
            tf.float32, [None, self.struc_size], name="struc_input")
        self.input_y = tf.placeholder(
            tf.float32, [None, self.cont_size], name="cont_input")

        # Homophily neighbors for structure and content
        self.input_x_neigh1 = tf.placeholder(
            tf.float32, [None, self.struc_size], name="struc_input_neigh1")
        self.input_x_neigh2 = tf.placeholder(
            tf.float32, [None, self.struc_size], name="struc_input_neigh2")
        self.input_y_neigh1 = tf.placeholder(
            tf.float32, [None, self.cont_size], name="cont_input_neigh1")
        self.input_y_neigh2 = tf.placeholder(
            tf.float32, [None, self.cont_size], name="cont_input_neigh2")
        ...
        with tf.variable_scope("struc_decoder"):
            for ii in range(len(self.decoder1)):
                ...
                xvec = tf.layers.dense(xvec, self.decoder1[ii],
                                       activation=tf.nn.leaky_relu, use_bias=True, name=layer_name)
            input_rec = tf.layers.dense(
                xvec, self.struc_size, activation=tf.nn.relu, use_bias=False, name="struc_final_layer")
        ...
        with tf.variable_scope("cont_decoder"):
            for ii in range(len(self.decoder2)):
                ...
                xvec = tf.layers.dense(xvec, self.decoder2[ii],
                                       activation=tf.nn.leaky_relu, use_bias=True, name=layer_name)
            input_rec = tf.layers.dense(
                xvec, self.cont_size, activation=tf.nn.relu, use_bias=False, name="cont_final_layer")
    ...
    def _discriminator(self, batch_x, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            x = tf.layers.dense(
                batch_x, 16, activation=tf.nn.relu, name='desc_layer_1')
    ...
    def _add_loss(self, batch_x, batch_y,
        ...
        with tf.variable_scope('loss'):
            ...
            self.loss1 = tf.reduce_sum(
                tf.square((5.0 * batch_x + 1e-2) - decoded_x), axis=1)  # 10 20 0.5 1

            #  Loss 2 cont
            self.loss2 = tf.reduce_sum(
                tf.square((5.0 * batch_y + 1e-2) - decoded_y), axis=1)  # 20 30 1 0.1
            ...
            tf.summary.scalar(
                "Total_Loss", self.encoder_loss + self.generator_loss)
            tf.summary.scalar("Structure_Loss", tf.reduce_mean(loss1))
            tf.summary.scalar("Content_Loss", tf.reduce_mean(loss2))
            tf.summary.scalar("Struct_Homophily_Loss",
                              tf.reduce_mean(self.loss4))
            tf.summary.scalar("Content_Homophily_Loss",
                              tf.reduce_mean(self.loss5))
            tf.summary.scalar("Discriminator_Loss",
                              tf.reduce_mean(self.disc_loss))
            tf.summary.scalar("AutoEncoder_Align_Loss",
                              tf.reduce_mean(self.loss3))
    ...
    def create_network(self):
        ...
        with tf.variable_scope('autoencoder'):
            ...
            self.struct_neigh1 = self._add_encoder_struc(
                self.input_x_neigh1, reuse=True)
            self.struct_neigh2 = self._add_encoder_struc(
                self.input_x_neigh2, reuse=True)
            self.cont_neigh1 = self._add_encoder_cont(
                self.input_y_neigh1, reuse=True)
            self.cont_neigh2 = self._add_encoder_cont(
                self.input_y_neigh2, reuse=True)
    ...
    def initialize_optimizer(self, config):
        ...
        self.global_step_desc = tf.Variable(
            0, name="global_step_desc", trainable=False)
        self.global_step_gen = tf.Variable(
            0, name="global_step_gen", trainable=False)

        ae_vars = tf.get_collection(
    ...
    def train_step(self, sess, feed_dict, start_align=False, train_disc=False, print_this=True):
        ...
        if start_align:
            if (train_disc):
                run_vars = [self.train_op_disc,
                            self.global_step_desc, self.disc_loss]
                for ii in range(1):
                    _, idx, dloss = sess.run(run_vars, feed_dict=feed)

                run_vars = [self.train_op_gen,
                            self.global_step_gen, self.generator_loss]
                for ii in range(5):
                    _, idx, gloss = sess.run(run_vars, feed_dict=feed)

        run_vars = [self.train_op, self.global_step,
                    self.encoder_loss, self.all_summary]
    ...
    def get_hidden(self, sess, x_batch, y_batch):
        ...
        struc_emb, cont_emb = sess.run(
            [self.struc_hid, self.cont_hid], feed_dict=feed)
